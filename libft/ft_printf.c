...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/24 02:52:19 by jmoraes-          #+#    #+#             */
/*   Updated: 2023/11/10 11:07:30 by jmoraes-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_conversion(va_list vl, const char format)
{
	...
	char_printed = 0;
	if (format == 'c')
		char_printed += ft_putchar2(va_arg(vl, int));
	else if (format == 's')
		char_printed += ft_putstr2(va_arg(vl, char *));
	else if (format == 'p')
...
int	ft_printf(const char *format, ...)
{
	...
	while (format[i])
	{
		if (format[i] == '%')
		{
			...
			char_printed += ft_putchar2(format[i]);
		i++;
	}
	va_end(vl);
	return (char_printed);
